...
pub fn init_database() {
    let conn = Connection::open(get_root() + "/etc/ps4/databases/ps4.db").expect("Failed to create package database");
...
pub fn return_owned_files(package: &String) -> Result<Vec<String>, rusqlite::Error> {
    let conn = Connection::open(get_root() + "/etc/ps4/databases/ps4.db")?;
...
pub fn search_for_package(package: &String) -> Result<String, PackageDBError> {
    ...
    for i in get_sources() {
        let conn = Connection::open(format!("{}/etc/ps4/databases/cache/{}.db", get_root(), i.name)).expect("Failed to create package database");
...
pub fn get_installed_package(package: &String) -> Result<InstalledPackages, PackageDBError> {
    let conn = Connection::open(get_root() + "/etc/ps4/databases/ps4.db").expect("Failed to open database");
...
pub fn get_remote_package(package: &String, repo: &String) -> Result<Package, PackageDBError> {
    let conn = Connection::open(format!("{}/etc/ps4/databases/cache/{}.db", get_root(), repo)).expect("Failed to open database");
...
pub fn get_all_installed() -> Vec<InstalledPackages> {
    let conn = Connection::open(format!("{}/etc/ps4/databases/ps4.db", get_root())).expect("Failed to open package database");
...
pub fn get_group(repo: &String, group: &String) -> Vec<Package> {
    let conn = Connection::open(format!("{}/etc/ps4/databases/cache/{}.db", get_root(), repo)).expect("Failed to open package database");
